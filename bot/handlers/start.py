...
Для начала работы выберите пункты ниже 👇'
	if isinstance(msg_or_cb, Message):
		await msg_or_cb.answer(start_text, reply_markup=await kb.main(msg_or_cb.from_user.id))
	else:
		await msg_or_cb.message.edit_text('Выберите пункты ниже 👇',
		                                  reply_markup=await kb.main(msg_or_cb.from_user.id))
